...
####    Take the train file for training data from the command line as the second argument.
	...
	for key,value in ham.items():
		ham_counter += value	
	print "spam conter",spam_counter		
	print "ham conter",ham_counter		
	...
	for key,values in ham.items():
		cond_prob_word_seeing_ham[key] = float(values)/float(ham_counter)
	#for keys,val in cond_prob_word_seeing_ham.items():
	#	print (keys,val)
	for key,values in spam.items():
		cond_prob_word_seeing_spam[key] = float(values)/float(spam_counter)
